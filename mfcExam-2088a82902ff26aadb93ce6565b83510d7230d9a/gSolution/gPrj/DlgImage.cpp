...
void CDlgImage::drawData(CDC* pDC)
{
	CRect rect;
	CPen penYellow;
	penYellow.CreatePen(PS_SOLID, 2, RGB(255, 255, 0));
	CPen penRed;
	...
	for (int i = 0; i < m_nDataCount; i++) {
		...
		pDC->MoveTo(m_ptData[i].x, m_ptData[i].y - 5);
		pDC->LineTo(m_ptData[i].x, m_ptData[i].y + 5);
		pDC->MoveTo(m_ptData[i].x - 5, m_ptData[i].y);
		pDC->LineTo(m_ptData[i].x + 5, m_ptData[i].y);
	}
}
